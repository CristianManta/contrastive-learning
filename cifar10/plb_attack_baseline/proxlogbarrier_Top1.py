...
def LogRegCriterion(x, y, model):
    """ The Logistic Regression Classification criterion

        x -> images (torch float tensor)
        y -> true labels (torch Long tensor)        
        model -> the trained logistic regression classifier that maps features to classes  """
...
class Attack():
    ...
    def __init__(self, model, criterion, norm=0,
                 verbose=True, **kwargs):
